...
local function GetPickupAction(inst, target)
    ...
    if valid_action.PICK then
        ...
        elseif target:HasTag('harvestable') then
            return ACTIONS.HARVEST
        elseif target:HasTag('readyforharvest') or (target:HasTag('notreadyforharvest') and target:HasTag('withered')) then
            return ACTIONS.HARVEST
        ...
        elseif target:HasTag('donecooking') and not target:HasTag('burnt') then
            return ACTIONS.HARVEST
        elseif
            tool ~= nil and tool:HasTag('unsaddler') and target:HasTag('saddled') and
                (not target.replica.health or not target.replica.health:IsDead())
         then
            return ACTIONS.UNSADDLE
        elseif
            tool ~= nil and tool:HasTag('brush') and target:HasTag('brushable') and
                (not target.replica.health or not target.replica.health:IsDead())
         then
            return ACTIONS.BRUSH
        elseif
            inst.components.revivablecorpse ~= nil and target:HasTag('corpse') and ValidateCorpseReviver(target, inst)
         then
            return ACTIONS.REVIVE_CORPSE
        end
    end
    if
        target.replica.inventoryitem ~= nil and target.replica.inventoryitem:CanBePickedUp() and
            not (target:HasTag('heavy') or target:HasTag('fire') or target:HasTag('catchable'))
     then
        return (inst.components.playercontroller:HasItemSlots() or target.replica.equippable ~= nil) and ACTIONS.PICKUP or
            nil
    end
end
...
local function MountedActionButton(inst, force_target)
    ...
    if inst:HasTag('cancatch') and not inst.components.playercontroller:IsDoingOrWorking() then
        ...
        elseif inst:GetDistanceSqToInst(force_target) <= 100 and force_target:HasTag('catchable') then
...
local function SGwilsonPostInit(self)
    if valid_action.JUMPIN then
        self.states['jumpin_pre'].onenter = function(inst)
            inst.components.locomotor:Stop()
            inst.AnimState:PlayAnimation(
                inst.components.inventory:IsHeavyLifting() and 'heavy_jump_pre' or 'jump_pre',
                false
            )
        ...
        self.states['jumpin'].onenter = function(inst, data)
            ...
            if
                inst.sg.statemem.target ~= nil and inst.sg.statemem.target:IsValid() and
                    inst.sg.statemem.target.components.teleporter ~= nil
             then
...
local function tradablefn(inst, doer, target, actions)
    if
        target:HasTag('trader') and not (target:HasTag('player') or target:HasTag('ghost')) and
        ...
        if right and inst.replica.container:IsOpenedBy(doer) then
            table.insert(
                ...
                doer.components.constructionbuilderuidata ~= nil and
                    doer.components.constructionbuilderuidata:GetContainer() == inst and
                    ACTIONS.APPLYCONSTRUCTION or
                    ACTIONS.WRAPBUNDLE
            )
        end
    elseif not inst:HasTag('burnt') and inst.replica.container:CanBeOpened() and doer.replica.inventory ~= nil then
...
function stewerfn(inst, doer, actions, right)
    if not inst:HasTag('burnt') then
        ...
        elseif
            right and
                (inst:HasTag('readytocook') or
                    (inst.replica.container ~= nil and inst.replica.container:IsFull() and
    ...
    if inst.components.health ~= nil then
        if (inst.components.saltlicker.salted or inst.components.sleeper:IsAsleep()) then
            inst.components.health:StartRegen(
                _G.TUNING.BEEFALO_HEALTH_REGEN * 3,
                _G.TUNING.BEEFALO_HEALTH_REGEN_PERIOD / 4
            )
        else
